    ...
    implements QueryRunner
{
     ...
     */
    async query(rawQuery: string, parameters?: any[]): Promise<any> {
        ...
        if (this.getMetadata != null) {
            ...
            if (metadata != null && Object.keys(metadata).length > 0) {
                const serializedMetadata = Object.entries(metadata)
                    ...
                    .join(",");
                const sanitizedSerializedMetadata = serializedMetadata.replace(
                    /\*\//g,
                    ""
                );
    ...
    protected buildSequenceName(
        ...
        if (seqName.length > this.connection.driver.maxAliasLength!)
            ...
            seqName = `${tableName.substring(0, 29)}_${columnName.substring(
                0,
                Math.max(29, 63 - tableName.length - 5)
            )}_seq`;
    ...
    protected escapeComment(comment?: string) {
        ...
        comment = comment.replace("'", "''").replace("\0", ""); // Null bytes aren't allowed in comments
